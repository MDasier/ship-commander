...
// Forma de nave para dibujo. Geometría heterogénea (algunas naves tienen
// cockpitRect / turretHardpoints, otras no), por eso los campos son opcionales.
interface ShipShape {
  body: number[][];
  engine: number[][];
  hpBarW: number;
...
function getShapeDef(type: string): ShipShape {
  return (SHIP_SHAPES as Record<string, ShipShape>)[type] || SHIP_SHAPES.fighter;
}

// Draw a ship polygon path (no fill/stroke — caller does that)
function buildShipPath(c: CanvasRenderingContext2D, type: string) {
...
function drawShipDetail(c: CanvasRenderingContext2D, type: string, player: { team?: string; dead?: boolean }) {
...
function drawShipPreviewInto(el: HTMLCanvasElement, type: string, shape: ShipShape, accent?: string) {
  const C = accent === "gold"
...
function drawShipPreview(canvas: HTMLCanvasElement | null, type: string, accent?: string) {
  const shape = (SHIP_SHAPES as Record<string, ShipShape>)[type];
